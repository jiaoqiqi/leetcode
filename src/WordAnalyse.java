...
public class WordAnalyse {
    ...
    private String[] keyWord = {"and", "begin", "const", "div",
            "do", "else", "end", "function", "if", "integer",
            "not", "or", "procedure", "program", "read",
            "real", "then", "type", "var", "while", "write"};

    public static void main(String[] args) throws IOException {
        WordAnalyse wordAnalyse = new WordAnalyse();
        wordAnalyse.readFile("a.txt");
        wordAnalyse.run();
    ...
    private void run() {
        ...
        for (int i = 0; i < strbuf.length; i++) {
            ...
            switch (strbuf[i]) {
                case ',':
                    System.out.println(strbuf[i] + "\t\t" + 23);
                    break;
                case ';':
                    System.out.println(strbuf[i] + "\t\t" + 24);
                    break;
                case '.':
                    System.out.println(strbuf[i] + "\t\t" + 26);
                    break;
                case '(':
                    System.out.println(strbuf[i] + "\t\t" + 27);
                    break;
                case ')':
                    System.out.println(strbuf[i] + "\t\t" + 28);
                    break;
                case '[':
                    System.out.println(strbuf[i] + "\t\t" + 29);
                    break;
                case ']':
                    System.out.println(strbuf[i] + "\t\t" + 30);
                    break;
                case '+':
                    System.out.println(strbuf[i] + "\t\t" + 34);
                    break;
                case '-':
                    System.out.println(strbuf[i] + "\t\t" + 35);
                    break;
                case '*':
                    System.out.println(strbuf[i] + "\t\t" + 45);
                    break;
                case '/':
                    System.out.println(strbuf[i] + "\t\t" + 46);
                    break;
                case '=':
                    System.out.println(strbuf[i] + "\t\t" + 38);
                    break;
                case ':': {
                    buf.delete(0, buf.length());
                    buf.append(strbuf[i]);
